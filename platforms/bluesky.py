    ...
    def get_latest_mentions(self, limit: int = 10) -> List[dict]:
        ...
        try:
            ...
            for notification in notifications.notifications:
                ...
                if self.debug:
                    print(
                        f"Found {reason} notification: {getattr(notification, 'uri', 'no uri')}"
                    )
                ...
                try:
                    ...
                    if hasattr(notification, "record") and notification.record:
                        ...
                        if self.debug:
                            print(
                                f"  Found text in notification.record: {text[:50]}..."
                            )
                    ...
                    if not text:
                        ...
                        posts_data = self.client.app.bsky.feed.get_posts(
                            uris=[post_uri]
                        )

                        if (
                            posts_data
                            and posts_data.posts
                            and len(posts_data.posts) > 0
                        ):
                            ...
                            if hasattr(post, "record") and post.record:
                                ...
                                if self.debug:
                                    print(
                                        f"  Successfully fetched post with text: {text[:50]}..."
                                    )
                            ...
                            if self.debug:
                                print("  ERROR: No post data returned from get_posts")
                            continue
                    else:
